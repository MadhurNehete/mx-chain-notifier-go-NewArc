...
func (rp *rabbitMqPublisher) PublishRevert(revertBlock data.RevertBlock) error {
	revertBlockBytes, err := rp.marshaller.Marshal(revertBlock)
	if err != nil {
		return err
	}

	return rp.publishFanout(rp.cfg.RevertEventsExchange.Name, revertBlockBytes)
}

// PublishFinalized will publish finalized event to rabbitmq
func (rp *rabbitMqPublisher) PublishFinalized(finalizedBlock data.FinalizedBlock) error {
	finalizedBlockBytes, err := rp.marshaller.Marshal(finalizedBlock)
	if err != nil {
		return err
	}

	return rp.publishFanout(rp.cfg.FinalizedEventsExchange.Name, finalizedBlockBytes)
}
...
// BroadcastRevert will publish revert event to rabbitmq
func (rp *rabbitMqPublisher) BroadcastRevert(revertBlock data.RevertBlock) error {
	return rp.PublishRevert(revertBlock)
}

// BroadcastFinalized will publish finalized event to rabbitmq
func (rp *rabbitMqPublisher) BroadcastFinalized(finalizedBlock data.FinalizedBlock) error {
	return rp.PublishFinalized(finalizedBlock)
}

// Broadcast will publish logs and events to rabbitmq
func (rp *rabbitMqPublisher) Broadcast(events data.BlockEvents) {
	rp.Publish(events)
}

// BroadcastTxs will publish txs event to rabbitmq
func (rp *rabbitMqPublisher) BroadcastTxs(blockTxs data.BlockTxs) {
	rp.PublishTxs(blockTxs)
}

// BroadcastScrs will publish scrs event to rabbitmq
func (rp *rabbitMqPublisher) BroadcastScrs(blockScrs data.BlockScrs) {
	rp.PublishScrs(blockScrs)
}

// BroadcastBlockEventsWithOrder will publish block events with order to rabbitmq
func (rp *rabbitMqPublisher) BroadcastBlockEventsWithOrder(blockTxs data.BlockEventsWithOrder) {
	rp.PublishBlockEventsWithOrder(blockTxs)
}

// BroadcastStateAccesses will publish block state accesses to rabbitmq
func (rp *rabbitMqPublisher) BroadcastStateAccesses(stateAccesses data.BlockStateAccesses) {
	rp.PublishStateAccesses(stateAccesses)
}

// Run returns nil
func (rp *rabbitMqPublisher) Run() error {
	return nil
}
